...
namespace DtCms.Web.Admin.Advertising
{
    public partial class BarAdd : DtCms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                JscriptMsg(350, 230, "错误提示", "<b>出现错误啦！</b>对不起，首页图片不存在或者已经被删除了。", "back", "Error");
                return;
            }
            if (!Page.IsPostBack)
            {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            bll.Add(model);
            //保存日志
            SaveLogs("[首页图片管理]增加首页图片内容：" + model.Title);
            JscriptPrint("首页图片发布成功啦！", "BarAdd.aspx?Pid=" + this.pid, "Success");
        }
    }
}
